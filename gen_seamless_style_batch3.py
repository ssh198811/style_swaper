...
def style_main3(pics_dir=[],style_dir=''):
    patch_size=1
    ...
    if pics_dir is not None:
        ...
        for file_path in pics_dir:
            file_path=file_path.replace(".dds",".jpg")
            if file_path.endswith(".jpg") is False:
                continue
            try:
                file_name=os.path.basename(file_path)
                parent_pth=os.path.dirname(file_path)
                file_path=parent_pth+'/style_transfer/expanded/expanded_transfer/'+file_name
                ##文件名
                ...
                print(s_name)
                save_dir = parent_pth + '/style_transfer/expanded/style_output/'
                if os.path.exists( f'{save_dir}{s_name}/' + file) is False:
                    ...
                    InfoNotifier.InfoNotifier.g_progress_info.append( f'{save_dir}{s_name}/' + file+'  exists')
            ...
            try:
                if os.path.exists(f'{save_dir}{s_name}/' + file) is False:
                    ...
                    # print(f'contentname:{file_name};file:{file}')
                    ...
                    InfoNotifier.InfoNotifier.g_progress_info.append(f'stylized image has saved into files: {save_dir}{s_name}/' + file)
